...
namespace RiftboundCalendar.Infrastructure.Filtering;

public static class HaversineFilter
{
    private const double EarthRadiusKm = 6371.0;

    public static IReadOnlyList<RiftboundEvent> Filter(
        IEnumerable<RiftboundEvent> events,
        double centerLat,
        double centerLng,
        double radiusKm)
    {
        return events
            .Where(e => Distance(centerLat, centerLng, e.Location.Latitude, e.Location.Longitude) <= radiusKm)
            .ToList();
    }
